...
export class CharactersComponent {
  ...
  async ngOnInit() {
    ...
    .then( (content) => {
      //this.characters[0].name = content;
    ...
    for(var i = 0; i < this.length; i++) {
      await fetch(this.ip + "ReturnCharacterSheetInfo?Username=" + this.username + "&CharacterPos" + i, {
        method: "GET",
    })
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        ...
        this.characters[0].name = content.charName;
    })
    .catch(error => {
        // Handle any errors that occur during the fetch request
        console.error(error);
